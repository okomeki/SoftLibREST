...
public class RestClient {
    ...
    public JSON2Value get2(String uri) throws IOException, URISyntaxException {
        return get2(new URI(baseuri + uri));
    }
     ...
     * @throws IOException
     */
    public JSON2Value get2(URI uri) throws IOException {
        HttpURLConnection conn = (HttpURLConnection) uri.toURL().openConnection();
    ...
    public JSON2Value post2(String uri, String... parameters) throws IOException, URISyntaxException {
        return post2(new URI(baseuri + uri), parameters);
    }
     ...
     * @throws IOException
     */
    public JSON2Value post2(URI uri, String... parameters) throws IOException {
        HttpURLConnection conn = (HttpURLConnection) uri.toURL().openConnection();
        // https限定
        conn.setRequestMethod("POST");
        conn.setRequestProperty("Accept", "application/json");
    ...
    private JSON2Value result2(HttpURLConnection conn) throws IOException {
        byte[] result = FileIO.binRead(conn.getInputStream());
        conn.disconnect();
        return (JSON2Value) JSON2.parse(result);
    }
}
